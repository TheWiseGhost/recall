...
class TestSplitting:
    async def test_empty_document_produces_no_chunks(self) -> None:
        chunker = FixedSizeChunker(chunk_size=10, overlap=2)
        assert await chunker.chunk(make_document("   \n\n  ")) == []

    async def test_short_document_is_one_chunk(self) -> None:
        chunker = FixedSizeChunker(chunk_size=100, overlap=10)
        chunks = await chunker.chunk(make_document("just a few words"))
        assert len(chunks) == 1
    ...
    async def test_respects_chunk_size(self) -> None:
        ...
        chunks = await chunker.chunk(make_document(words(35)))
        assert [len(c.content.split()) for c in chunks] == [10, 10, 10, 5]

    async def test_overlap_repeats_trailing_tokens(self) -> None:
        chunker = FixedSizeChunker(chunk_size=10, overlap=3, token_counter=WhitespaceTokenCounter())
        chunks = await chunker.chunk(make_document(words(20)))
    ...
    async def test_never_splits_a_word(self) -> None:
        chunker = FixedSizeChunker(chunk_size=5, overlap=1)
        for chunk in await chunker.chunk(make_document(words(50))):
            for token in chunk.content.split():
                assert token.startswith("w") and token[1:].isdigit()

    async def test_covers_every_token(self) -> None:
        content = words(53)
    ...
    async def test_terminates_on_a_single_oversized_token(self) -> None:
        """One word longer than the budget must still be emitted, once."""
        content = "a" * 5000
        chunker = FixedSizeChunker(chunk_size=16, overlap=4)
        chunks = await chunker.chunk(make_document(content))
        assert len(chunks) == 1
...
class TestChunkMetadata:
    async def test_source_metadata_is_preserved(self) -> None:
        chunker = FixedSizeChunker(chunk_size=10, overlap=2)
        chunks = await chunker.chunk(make_document(words(30), file_type="md", author="docs"))
        assert all(c.metadata["file_type"] == "md" for c in chunks)
        assert all(c.metadata["author"] == "docs" for c in chunks)

    async def test_records_strategy_provenance(self) -> None:
        chunker = FixedSizeChunker(chunk_size=10, overlap=2)
        chunk = (await chunker.chunk(make_document(words(30))))[0]
        assert chunk.metadata["chunker"] == "fixed"
        assert chunk.metadata["chunker_params"] == {"chunk_size": 10, "overlap": 2}

    async def test_token_counts_are_recorded(self) -> None:
        chunker = FixedSizeChunker(chunk_size=10, overlap=0, token_counter=WhitespaceTokenCounter())
        chunks = await chunker.chunk(make_document(words(25)))
        assert [c.token_count for c in chunks] == [10, 10, 5]


class TestChunkIdentity:
    async def test_same_document_yields_same_chunk_ids(self) -> None:
        document = make_document(words(40))
        chunker = FixedSizeChunker(chunk_size=8, overlap=2)
        assert [c.id for c in await chunker.chunk(document)] == [
            c.id for c in await chunker.chunk(document)
        ]

    async def test_changed_content_yields_different_chunk_ids(self) -> None:
        chunker = FixedSizeChunker(chunk_size=8, overlap=2)
        first = await chunker.chunk(make_document(words(40)))
        second = await chunker.chunk(make_document(words(40) + " extra"))
        assert first[-1].id != second[-1].id

    async def test_all_chunks_belong_to_their_document(self) -> None:
        document = make_document(words(40))
        for chunk in await FixedSizeChunker(chunk_size=8, overlap=2).chunk(document):
